...
use magic_linux::{
    InputReader, OneFingerSwipeRecognizer, TouchTracker, discover_magic_mice, gesture::Direction,
};
...
fn main() -> ExitCode {
    ...
    println!("Magic Mouse 1-finger swipe input debugger");
    ...
    println!("Listening for 1-finger horizontal swipes...");
    println!("LEFT  = Navigation Back");
    println!("RIGHT = Navigation Forward");
    println!();

    let mut touch_tracker = TouchTracker::new();
    let mut swipe_recognizer = OneFingerSwipeRecognizer::new();
    ...
    loop {
        ...
        for event in events {
            ...
            if let Some(swipe) = swipe_recognizer.process(&normalized) {
                ...
                println!("SWIPE | 1 finger | {direction}");
            }
        }
    }
}
